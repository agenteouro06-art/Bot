...
SYSTEM_PROMPT = (
“Eres CLAW, sistema multiagente especializado en automatizacion con n8n.\n\n”
“AGENTES QUE EJECUTAS:\n”
“[ANALISTA] Detecta tipo de negocio y objetivo\n”
“[ARQUITECTO] Decide nodos n8n y conexiones\n”
“[DISENADOR] Crea el JSON completo del workflow\n”
“[VALIDADOR] Verifica conexiones y logica\n”
...
“2. Plan de construccion\n”
“3. APIs y credenciales necesarias\n”
“4. Descripcion del workflow\n”
“5. Valor comercial estimado\n”
“6. El JSON completo entre `json y `\n\n”
“Si faltan credenciales pideselas al usuario ANTES de generar.\n”
“Responde siempre en espanol.”
)

def llamar_ia(mensaje, credenciales=””):
prompt = mensaje
if credenciales:
...
try:
    ...
    return {"texto": "Error llamando a la IA: " + str(e), "workflow_json": None}
...
try:
r = requests.post(N8N_URL + “/api/v1/workflows”, headers=n8n_headers(), json=workflow, timeout=15)
r.raise_for_status()
return {“ok”: True, “data”: r.json()}
except Exception as e:
return {“ok”: False, “error”: str(e)}

def activar_workflow_n8n(wf_id):
try:
r = requests.patch(N8N_URL + “/api/v1/workflows/” + wf_id, headers=n8n_headers(), json={“active”: True}, timeout=10)
r.raise_for_status()
return {“ok”: True}
except Exception as e:
return {“ok”: False, “error”: str(e)}

def ejecutar_workflow_n8n(wf_id):
try:
r = requests.post(N8N_URL + “/api/v1/workflows/” + wf_id + “/run”, headers=n8n_headers(), timeout=30)
r.raise_for_status()
return {“ok”: True, “data”: r.json()}
except Exception as e:
return {“ok”: False, “error”: str(e)}

def obtener_ejecuciones_n8n(wf_id):
try:
r = requests.get(N8N_URL + “/api/v1/executions?workflowId=” + wf_id + “&limit=1”, headers=n8n_headers(), timeout=10)
r.raise_for_status()
return {“ok”: True, “data”: r.json()}
except Exception as e:
return {“ok”: False, “error”: str(e)}

def listar_workflows_n8n():
try:
r = requests.get(N8N_URL + “/api/v1/workflows”, headers=n8n_headers(), timeout=10)
r.raise_for_status()
return {“ok”: True, “data”: r.json()}
except Exception as e:
return {“ok”: False, “error”: str(e)}

def obtener_workflow_n8n(wf_id):
try:
r = requests.get(N8N_URL + “/api/v1/workflows/” + wf_id, headers=n8n_headers(), timeout=10)
r.raise_for_status()
return {“ok”: True, “data”: r.json()}
except Exception as e:
return {“ok”: False, “error”: str(e)}
...
if status == “success”:
msg = “OK - Workflow funciona correctamente.\nVer: “ + N8N_URL + “/workflow/” + wf_id + “/executions”
elif status == “error”:
msg = “ERROR - Revisa las credenciales en n8n.\nVer: “ + N8N_URL + “/workflow/” + wf_id + “/executions”
else:
msg = “Estado: “ + status + “\nVer: “ + N8N_URL + “/workflow/” + wf_id + “/executions”
else:
msg = “Sin ejecuciones. El workflow puede necesitar un Webhook externo para activarse.”
else:
msg = “No se pudo verificar la ejecucion.”
...
async def procesar_solicitud(update, context, texto, credenciales=””):
...
if resultado["workflow_json"]:
    ...
    await update.message.reply_text(
        "No se genero JSON. Proporciona las credenciales indicadas arriba y escribelas aqui."
    )
```
...
if data in ("crear_workflow", "crear_y_probar"):
    ...
    if resultado["ok"]:
        ...
        msg = "Workflow creado: " + wf.get("name", "Sin nombre") + "\nID: " + str(wf_id) + "\nURL: " + N8N_URL + "/workflow/" + str(wf_id)
        ...
        await context.bot.send_message(chat_id=chat_id, text="Error al crear: " + resultado["error"])
    ...
    if r["ok"]:
        await query.edit_message_text("Workflow " + str(wf_id) + " activado correctamente.")
    else:
        await query.edit_message_text("Error activando: " + r["error"])
    ...
    for i in range(0, len(json_str), 3900):
        ...
        await context.bot.send_message(chat_id=chat_id, text="```json\n" + chunk + "\n```", parse_mode="Markdown")
    ...
    if resultado["ok"]:
        ...
        for wf in workflows[:10]:
            ...
            texto += "[" + estado + "] " + str(wf["id"]) + " - " + wf["name"] + "\n"
            kb.append([InlineKeyboardButton("Clonar: " + wf["name"][:25], callback_data="clonar_" + str(wf["id"]))])
        await query.edit_message_text(texto, reply_markup=InlineKeyboardMarkup(kb))
    else:
        await query.edit_message_text("Error: " + resultado["error"])
    ...
    if resultado["ok"]:
        ...
        wf_clon["name"] = "[CLON] " + wf_original.get("name", "Workflow")
        ...
        await context.bot.send_message(
            chat_id=chat_id,
            text="Listo para clonar: " + wf_clon["name"],
            reply_markup=InlineKeyboardMarkup(kb)
        )
    else:
        await context.bot.send_message(chat_id=chat_id, text="Error clonando: " + resultado["error"])
```
...
“Escribeme que flujo necesitas y lo construyo.\n\n”
“Comandos:\n”
...
await update.message.reply_text(“Error: “ + resultado[“error”])
...
await update.message.reply_text(
“Listo para clonar: “ + wf_clon[“name”],
reply_markup=InlineKeyboardMarkup(kb)
)
else:
await update.message.reply_text(“Error: “ + resultado[“error”])
...
if r[“ok”]:
await update.message.reply_text(“Workflow “ + context.args[0] + “ activado.”)
else:
